...
/-- Preservation: If e has type τ, and e' lies in the support of one small-step of e, then e' also has type τ. -/
...
noncomputable def step {τ : Ty} (e : ExprsOfType τ) : Dist (ExprsOfType τ) := by
  classical
  let dτ : ExprsOfType τ :=
    ⟨Expr.diverge, HasType.diverge (Γ := Ctx.empty) (τ := τ)⟩
  exact
    (Untyped.step e.1).map (fun e' =>
      if h : HasType Ctx.empty e' τ then
        (⟨e', h⟩ : ExprsOfType τ)
      else
        -- dummy branch, because .map must take a total function. This is where the preservation theorem comes in.
        dτ)
